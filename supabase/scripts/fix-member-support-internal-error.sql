-- Harden Member Support RPCs: avoid 500s when optional tables are missing or auth rows are orphaned.
...
BEGIN
  ...
  IF to_regclass('public.member_email_events') IS NOT NULL THEN
...
DECLARE
  ...
  v_import JSONB;
  v_email_events JSONB := '[]'::jsonb;
  v_support_notes JSONB := '[]'::jsonb;
  v_result JSONB;
BEGIN
  IF NOT is_admin() THEN
    RAISE EXCEPTION 'Admin access required';
  ...
  IF to_regclass('public.member_support_notes') IS NOT NULL THEN
    SELECT coalesce(
      (
        ...
        FROM (
          ...
          LEFT JOIN auth.users au ON au.id = sn.author_id
