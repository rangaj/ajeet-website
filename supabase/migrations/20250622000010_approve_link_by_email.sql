-- Resolve auth user by submitted_email on approve when user_id was not linked yet.
...
-- Self-heal when an approved alum signs in before user_id was linked at approval time.
CREATE OR REPLACE FUNCTION link_approved_alumni_self()
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
  ...
  member_rec alumni_members%ROWTYPE;
BEGIN
  IF uid IS NULL OR email = '' THEN
    RETURN false;
  END IF;

  SELECT am.* INTO member_rec
  FROM alumni_members am
  WHERE am.status = 'approved'
    AND (am.user_id IS NULL OR am.user_id = uid)
    AND (
      lower(trim(coalesce(am.email, ''))) = email
      OR EXISTS (
        SELECT 1 FROM approval_requests ar
        WHERE ar.roll_number = am.roll_number
          AND ar.status = 'approved'
          AND lower(trim(ar.submitted_email)) = email
      )
    )
  ...
  IF member_rec.user_id IS DISTINCT FROM uid THEN
    UPDATE alumni_members SET user_id = uid, updated_at = now()
    WHERE id = member_rec.id AND user_id IS NULL;
  END IF;

  UPDATE approval_requests SET user_id = uid, updated_at = now()
  WHERE roll_number = member_rec.roll_number
    AND status = 'approved'
    AND user_id IS NULL
    AND lower(trim(submitted_email)) = email;

  UPDATE profiles SET member_status = 'approved', updated_at = now()
  WHERE id = uid AND member_status IS DISTINCT FROM 'approved';

  RETURN true;
END;
$$;

GRANT EXECUTE ON FUNCTION link_approved_alumni_self TO authenticated;
